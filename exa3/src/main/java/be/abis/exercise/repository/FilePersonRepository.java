...
public class FilePersonRepository implements PersonRepository {
	...
	private DateTimeFormatter formatBirthDate = DateTimeFormatter.ofPattern("d/MM/yyyy");
		...
		try {
			...
			while ((s = br.readLine()) != null) {
				...
				if (!vals[0].equals("")) {
					...
					p.setBirthDate(!vals[3].equals("null") ? LocalDate.parse(vals[3], formatBirthDate) : null);
	...
	private StringBuilder parsePerson(Person p) {
		...
		String bd = p.getBirthDate().format(formatBirthDate);
		System.out.println(bd);
		sb.append(p.getPersonId() + ";").append(p.getFirstName() + ";").append(p.getLastName() + ";")
				.append(bd.equals(null) ? LocalDate.parse("01/01/1900", formatBirthDate) : bd + ";")
